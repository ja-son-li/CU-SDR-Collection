...
for PRN = settings.acqSatelliteList
    ...
    %repeat for coherent integration
    COHlocalE5aQCode = [repmat(localE5aQCode,1,settings.acqCohCodePeriods) zeros(1,samplesPerCode*settings.acqCohCodePeriods)];
    COHlocalE5aICode = [repmat(localE5aICode,1,settings.acqCohCodePeriods) zeros(1,samplesPerCode*settings.acqCohCodePeriods)];
    
    %--- Perform DFT of PRN code ------------------------------------------
    E5aICodeFreqDom = conj(fft(COHlocalE5aICode));
    E5aQCodeFreqDom = conj(fft(COHlocalE5aQCode));
    
    % NEW 6091 Final Project Circshift algorithm---------------------------
    freqIndex = 0;
    for freqBinIndex = 1:numberOfFreqBins

        freqIndex = freqIndex + 1;
        nonCohSummedResult = zeros(1,settings.acqCohFFTLength);
        
        coarseFreqBin(freqIndex) = settings.IF + settings.acqSearchBand - settings.acqSearchStep * (freqIndex - 1);
        
        for nonCohIndex = 1:settings.acqNonCohCodes
            longSignalChunk = longSignal((nonCohIndex-1)*settings.acqCohFFTLength+1:(nonCohIndex)*settings.acqCohFFTLength*2);
            longSignalFFT = fft(longSignalChunk);
            I_temp = real(longSignalFFT);
            Q_temp = imag(longSignalFFT);
            I_shift = circshift(I_temp,[0,freqBinIndex]);
            Q_shift = circshift(Q_temp,[0,freqBinIndex]);
            longSignalFFT_shift = fft(I_shift + 1i*Q_shift);

            E5aICohCorr = longSignalFFT_shift .*E5aICodeFreqDom;
            E5aQCohCorr = longSignalFFT_shift .*E5aQCodeFreqDom;

            nonCohSummedResult = nonCohSummedResult + (abs(ifft(E5aICohCorr)) + abs(ifft(E5aQCohCorr)));
        end
        results(freqIndex,:) = nonCohSummedResult;
    end
