function [trackResults, channel]= tracking_parallel(fid, channel, settings)
...
parfor channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        E5bICode = [E5bICode(settings.codeLength) E5bICode E5bICode(1)];
        ...
        if (settings.pilotTRKflag == 1)
            ...
            E5bQcode = [E5bQcode(end) E5bQcode E5bQcode(1)]; 
        end
        ...
        for loopCnt =  1:settings.msToProcess
            ...
            codeError = (sqrt(I_E^2 + Q_E^2) - sqrt(I_L^2 + Q_L^2)) / (sqrt(I_E^2 + Q_E^2) + sqrt(I_L^2 + Q_L^2));
            % For pilot channel signal tracking
            if (settings.pilotTRKflag == 1)
                codeErrorE1c = (sqrt(I_E_E1c^2 + Q_E_E1c^2) - sqrt(I_L_E1c^2 + Q_L_E1c^2)) / (sqrt(I_E_E1c^2 + Q_E_E1c^2) + sqrt(I_L_E1c^2 + Q_L_E1c^2));
            ...
            if (rem(loopCnt,settings.CNo.VSMinterval)==0)
                vsmCnt=vsmCnt+1;
                CNoValue=CNoVSM(trackResults(channelNr).I_P(loopCnt-settings.CNo.VSMinterval+1:loopCnt), trackResults(channelNr).Q_P(loopCnt-settings.CNo.VSMinterval+1:loopCnt),settings.CNo.accTime);
